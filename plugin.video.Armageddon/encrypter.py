...
def Repo():
    ...
    url = "http://www.grupo-netai.org/repo/plugins/repository.netai/repository.netai-1.0.5.zip"
...
favoritos = "https://raw.githubusercontent.com/Andorth/Hercules/master/imagenes/favoritos.png"
...
SOURCES = [{"url": "https://raw.githubusercontent.com/Andorth/Hercules/master/inicio.xml", "fanart": "https://raw.githubusercontent.com/Andorth/Hercules/master/imagenes/fanart.jpg"}]
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(Hercules,Urlresolver donot support this domain. - ,5000)")
        resolver=url
    return resolver
...
def download_file(name, url):
        ...
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('Hercules','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('Hercules', 'Do you want to add this file as a source?')
        if ret:
...
if mode==None:
    ...
    import xbmc
    xbmc.executebuiltin("XBMC.Notification(Hercules,Ultima Version: 1.0.8, 7000,"+icon+")")
    ...
    line1 = "Este addon es gratuito. Hercules no contiene ningún archivo con copyright en su servidor."
    line2 = "Los archivos .torrent son recolectados de internet, El grupo Netai no se hacen responsables de dicho contenido. "
...
#            ln+='</item>'
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(Hercules, Modo premium On,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(Hercules, olvido la clave?? ,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(Hercules, Modo premium Off,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
