...
@limited
def ConditionalExpression(type, test, consequent, alternate, comments=None): # caused plenty of problems in my home-made translator :)
...
def FunctionDeclaration(type, id, params, defaults, body, generator, expression, comments=None, source=None):
    if generator:
...
def FunctionExpression(type, id, params, defaults, body, generator, expression, comments=None, source = None):
    if generator:
        raise NotImplementedError('Generators not supported')
    if defaults:
        raise NotImplementedError('Defaults not supported')
