...
class Js(object):
    def __new__(cls, val=None, Clamped=False, source=None):
        if not source:
            return cls.do_js(val, Clamped)
        else:
            return object.__new__(cls, source)

    def __init__(self, source):
        self.source = source

    def __call__(self, val, Clamped=False):
        return self.do_js(val, Clamped, self.source)

    @staticmethod
    def do_js(val, Clamped=False, source=None):
        ...
        if isinstance(val, PyJs):
            ...
            return PyJsFunction(val, FunctionPrototype, source=source)
