...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(plugin.video.music)'):
        return os.path.join(xbmcaddon.Addon('plugin.video.music').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(plugin.video.music)') else setting('appearance.alt').lower()
    return appearance


def music():
    execute('RunPlugin(plugin://plugin.video.music)')
...
def metaFile():
    if condVisibility('System.HasAddon(plugin.video.music)'):
        return os.path.join(xbmcaddon.Addon('plugin.video.music').getAddonInfo('path'), 'resources', 'data', 'meta.db')
