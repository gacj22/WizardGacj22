...
class DialogSelect(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        ...
        for item in self.items:
            ...
            label1 = "(%s/%s) [%s] %s" % (item["seeds"], item["peers"], item["size"], quality)
            label2 = item["parser"]
            label3 = item["name"]
            icon = item["icon"]
            played = item["played"]
            self.addItem(label1, label2, label3, icon, played)
    ...
    def addItem(self, label1="", label2="", label3="", icon=None, played=False):
        """
        Adding an item
        ...
        item.setProperty('Label3', label3)
