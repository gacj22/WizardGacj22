...
def acestreams(name,iconimage,chid):
	torrdir = settings.getSetting("torrent_path")
	if torrdir != "":
		fichero = os.path.join(torrdir,"Plexus-Streams.m3u")
	...
	if settings.getSetting('engine_app') != '2' and settings.getSetting('engine_app') != '3' and settings.getSetting('engine_app') != '4':
		...
		elif settings.getSetting('engine_app') == '4':
			xbmc.executebuiltin( "PlayMedia("+"plugin://plugin.video.torrentin/?uri=" + chid+")" )

def acestreams_builtin(name,iconimage,chid):
    if xbmc.getCondVisibility('system.platform.windows'):
		if settings.getSetting('engine_app') == '4':
			xbmc.executebuiltin( "PlayMedia("+"plugin://plugin.video.torrentin/?uri=" + chid+")" )
		else:
			try:
				import _winreg
