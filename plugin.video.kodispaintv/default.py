...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.kodispaintvmod-%s]: %s" %(addon_version, string))
            ...
            try:
                ...
                if len(menu('link')) >0:
                    ...
                    for i in menu('sportsdevil'):
                        if not i.string == None:
                            sportsdevil = 'plugin://plugin.video.SportsDevil/?mode=1&amp;menu=catcher%3dstreams%26url=' +i.string
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(KodiSpainTV,Urlresolver donot support this domain. - ,5000)")
        resolver=url
    return resolver
...
if not playitem =='':
    ...
    name,url,regexs=getMenus(s,None,dontLink=True)
    mode=117 
if mode==None:
