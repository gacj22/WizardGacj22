...
jezyk = str(my_addon.getSetting('jezyk'))
BASEURL='http://www.sport365.live/%s'%jezyk
...
def getChannels(addheader=False):
	...
	for i in range(len(ids[:-1])):
		...
		online = '[COLOR lime]► [/COLOR]' if subset.find('/images/types/dot-green-big.png')>0 else '[COLOR orangered]■ [/COLOR]'
		if links and title2:
			...
			qualang = '[COLOR orange]%s-%s[/COLOR]' % (lang, quality)
			title = '[B][COLOR khaki]%s%s  : [/COLOR][/B] [COLOR gold][B]%s[/B][/COLOR] %s, %s'%(online,etime,title1,qualang,title2[0])
			code = '[B][COLOR lightgreen]'+quality+'-'+lang+'[/COLOR][/B]'
			plot = '[B][COLOR khaki]%s%s  : [/COLOR][/B] [COLOR gold][B]%s[/B][/COLOR][CR]%s[CR]%s'%(online,etime,title1,title2[0],qualang)
			#plot='%s[CR]'%(title)
			out.append({"title": title, "url": url,'image':title2[0], "code": code, "plot": plot})
	return out
...
def getStreams(url):
    ...
    for i, s in enumerate(set(sources)):
        ...
        title = '[COLOR lime]► [/COLOR][B][COLOR khaki]Link %d[/COLOR][/B] ' % (i+1)
